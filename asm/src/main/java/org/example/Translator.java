...
public class Translator {
    ...
    public static String translate(String t1, int cln) {
        String stringCode = "111";

        if (t1.charAt(0) == '@') {
            String target = t1.substring(1);
            String binaryTarget;

            if (Character.isDigit(target.charAt(0))) {
                int number = Integer.parseInt(target);
                binaryTarget = String.format("%15s", Integer.toBinaryString(number)).replace(' ', '0');
            } else {
                if (!SymbolTable.contains(target)) {
                    SymbolTable.addEntry(target, cln);
                    return "0" + String.format("%15s", Integer.toBinaryString(cln)).replace(' ', '0') + "++";
                }
                binaryTarget = String.format("%15s", Integer.toBinaryString(SymbolTable.getAddress(target))).replace(' ', '0');
            }

            return "0" + binaryTarget;
            ...
            if (t1.contains(";")) {
                targetJump = t1.substring(t1.length() - 3);
                targetComp = t1.substring(0, t1.indexOf(";"));
            } else if (t1.contains("=")) {
                targetDest = t1.substring(0, t1.indexOf("="));
                targetComp = t1.substring(t1.indexOf("=") + 1);
            } else {
                targetComp = t1;
            }
